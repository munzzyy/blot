...
#   dist/blot-<version>.apk  developer-signed universal APK: GitHub release,
#                                F-Droid reproducible verification, sideload
#   dist/blot-<version>.aab  upload-key-signed bundle for Play Console
#
# Never run by CI. The keystore lives outside the repo and its password stays
# in the system keyring (secret-tool lookup service blot-keystore key
...
KSPW=$(secret-tool lookup service blot-keystore key upload) || {
  echo "no keystore password in the keyring (service blot-keystore key upload)"; exit 1;
}
export KSPW
...
APK_OUT="dist/blot-$VERSION.apk"
AAB_OUT="dist/blot-$VERSION.aab"
...
# releases/latest/download/blot.apk across versions.
cp "$APK_OUT" dist/blot.apk

echo "== artifacts =="
sha256sum "$APK_OUT" "$AAB_OUT" dist/blot.apk
